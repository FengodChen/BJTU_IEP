...
void predict_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, int* newImg_flag, int* hadYolo_flag)
{
    float thresh = .5;
    float hier_thresh = .5;
    list *options = read_data_cfg(datacfg);
...
/*
void predict_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    while(1){
        ...
        get_detections(im, dets, nboxes, thresh, names, alphabet, l.classes, ddata);
        saveVehicleData("/Share/vehicle_data/location.db", ddata, nboxes);
        setVehicleFlag("/Share/vehicle_data/location.db", "hadYOLO", 1);
        free_detections(dets, nboxes);
#ifdef OPENCV
            make_window("predictions", 512, 512, 0);
            show_image(im, "predictions", 0);
